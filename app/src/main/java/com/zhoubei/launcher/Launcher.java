...
import com.zhoubei.launcher.DropTarget.DragObject;
...
public final class Launcher extends Activity
    ...
    // 在onCreate中设置状态，然后设置onNewIntent，在某些情况下会导致滚动问题(因为工作区可能还没有测量)和额外的工作。
    // 相反，只需保存我们需要恢复启动程序的状态，并在onResume中提交它。
    ...
    // it from the context.
    // 我们只希望获得一次SharedPreferences，因为每次我们从上下文获得它时，它都会执行FS stat。
    private SharedPreferences mSharedPrefs;

    // Holds the page that we need to animate to, and the icon views that we need to animate up
    // when we scroll to that page on resume.
    // 保存我们想让动画到达的页面以及图标，当我们滑动到这个页面在onresume时
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // LauncherModel load.
        // 如果我们得到一个onCreate，我们可以先抢占onResume，并将 mPaused 置为false
        // 这也确保了下面的任何同步绑定不会重新触发另一个LauncherModel负载。
        mPaused = false;
        ...
        if (!mRestoring) {
            if (sPausedFromUserAction) {
                ...
                // they return.
                // 如果用户离开了启动器，那么我们应该在它们返回时异步加载项。
                mModel.startLoader(true, -1);
            } else {
                // We only load the page synchronously if the user rotates (or triggers a
                // configuration change) while launcher is in the foreground
                // 加载应用图标的主要方法。如果用户旋转(或触发配置更改)，而启动程序位于前台，则我们只同步加载页面。
...
//        }
    ...
    void showAllApps(boolean animated) {
        ...
        mAppsCustomizeTabHost.requestFocus();   // 获取焦点

        // Change the state *after* we've called all the transition code
        mState = State.APPS_CUSTOMIZE;  // 改变当前的状态

        // Pause the auto-advance of widgets until we are out of AllApps
        mUserPresent = false;
        updateRunning();    // 移出定时runnable
        closeFolder();
